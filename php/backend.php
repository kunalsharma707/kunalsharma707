<?php
	...
	{   
	    ...
	    $value=array(array('string','mock_test_db.mock_test_tbl','name','Name as name,id,Email as email,Phone as phone,Gender as gender','id'));
	    $query_data=array();
	...
	Class ManageApp {
		...
		{   
			...
			$extra_slots_entry= mysqli_prepared_query($connection_mock_chat,$query,"ii",$params);
		        ...
		        {
					$res_here=[];
					for($i=0;$i<count($extra_slots_entry);$i++){

						$val=$extra_slots_entry[$i];
